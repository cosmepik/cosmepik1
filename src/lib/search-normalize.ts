...
/**
 * セット・入り表記を除去。
 * 「N種セット」「3色セット」「2点セット」「10本入り」「5個入」など、
 * 数字 ＋ 単位 ＋ （セット/入り/入）の組み合わせを対象にする。
 * 単体の「セット」は商品名本体に含まれる可能性があるため除外しない。
 *
 * 対象単位: 種 種類 色 点 個 本 袋 箱 枚 包 組
 * 対象末尾: セット 入り 入（省略可）
 *
 * 例: ３色セット / 3種セット / 2点セット / 10本入り / 5個入
 */
const SET_COUNT_PATTERN =
  /[0-9０-９]+\s*(?:種類?|色|点|個|本|袋|箱|枚|包|組)\s*(?:セット|入り?)?/gi;
...
export function cleanseItemName(name: string): string {
  ...
  // 5c-set. 3種セット / 3色セット / 10本入り などセット・入り表記
  s = s.replace(SET_COUNT_PATTERN, " ").trim();
